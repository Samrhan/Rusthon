...
| `compiler/generators/ndarray.rs` | NumPy ndarray codegen | `from_list`, `zeros`/`ones`/`arange`, `binop`, reductions (`reduce_sum`/`mean`/`max`/`min`/`prod`/`dot`), `unary_map` (ufuncs), `store_index`, `slice`, `print_array` |
